...
def main() -> int:
    print("the contract you implemented:")
    print(CONTRACT)

    if "..." in CONTRACT:
        print("[warning] CONTRACT still contains the template dots -- paste their\n"
              "          contract in first, word for word. Implementing against a\n"
              "          half-copied contract measures nothing.\n")
    if len(THEIR_CASES) < 2:
        print("[warning] THEIR_CASES holds the single example from the template.\n"
              "          Paste the case list from their validate_task4.py, or you are\n"
              "          testing your own expectations, not their contract.\n")
    ...
    print(header)
    print("-" * (UTTERANCE + GOT + EXPECTED + 2))
    differences = 0
    for utterance, expectation in THEIR_CASES:
        ...
        print(f"{utterance[:UTTERANCE - 1]:{UTTERANCE}s} "
              f"{shown[:GOT - 1]:{GOT}s} {expectation[:EXPECTED]}")
    ready = len(THEIR_CASES) >= 2 and "..." not in CONTRACT
    if not ready:
        print("\nNothing to compare yet -- paste their contract and their case list first.")
    elif differences:
        print(f"\n{differences} of {len(THEIR_CASES)} rows came out differently. That is not a\n"
        ...
        print("\nEvery row came out the way they expect. Now the harder question: could you\n"
