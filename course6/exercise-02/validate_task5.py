...
def main() -> None:
    for title, turns, expectation in DIALOGS:
        log.plain()
        log.rule(width=100, char="=")
        log.title(f"dialog: {title}")
        log.detail(f"you expect: {expectation}", indent="")
        log.rule(width=100, char="=")
        ...
        for turn in turns:
            ...
            log.detail(f"slots={state['slots']} expected={state['expected']!r} "
                       f"ended={state['ended']}")
            if state["ended"]:
                break
        log.result(f"  --> order_id: {state['order_id']}")
